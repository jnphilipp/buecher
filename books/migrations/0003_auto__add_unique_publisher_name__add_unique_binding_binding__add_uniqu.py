...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        ...
        db.create_unique(u'books_publisher', ['name'])

        # Adding unique constraint on 'Binding', fields ['binding']
        db.create_unique(u'books_binding', ['binding'])

        # Adding unique constraint on 'Language', fields ['language']
        db.create_unique(u'books_language', ['language'])

        # Adding unique constraint on 'Series', fields ['name']
        db.create_unique(u'books_series', ['name'])

        # Adding unique constraint on 'Person', fields ['firstname', 'lastname']
        db.create_unique(u'books_person', ['firstname', 'lastname'])


    def backwards(self, orm):
        # Removing unique constraint on 'Person', fields ['firstname', 'lastname']
        db.delete_unique(u'books_person', ['firstname', 'lastname'])

        # Removing unique constraint on 'Series', fields ['name']
        db.delete_unique(u'books_series', ['name'])

        # Removing unique constraint on 'Language', fields ['language']
        db.delete_unique(u'books_language', ['language'])

        # Removing unique constraint on 'Binding', fields ['binding']
        db.delete_unique(u'books_binding', ['binding'])

        # Removing unique constraint on 'Publisher', fields ['name']
        db.delete_unique(u'books_publisher', ['name'])


    models = {
        u'books.binding': {
            'Meta': {'ordering': "('binding',)", 'object_name': 'Binding'},
            'binding': ('django.db.models.fields.TextField', [], {'unique': 'True'}),
